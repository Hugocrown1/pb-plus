...
const ServiceRemoCard = ({ children, href, title, icon, style }) => {
  return (
    <article
      className={`group bg-[#FCFFFC] ${style} h-full xl:py-0 py-2 overflow-hidden text-center shadow-lg hover:bg-[#941B0C] hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all`}
    >
      <Link href={href}>
        <div className="relative flex w-full h-full items-center justify-center text-xl xl:text-2xl">
          ...
          <div className="absolute inset-y-[50%] inset-x-[15%]  bg-[#FCFFFC] w-[90%] h-[90%] transition-all group-hover:-rotate-45 group-hover:bg-[#621708]"></div>
        </div>
      </Link>
    </article>
  );
